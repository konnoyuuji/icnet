...
class MapillaryGenerator(Sequence):
    def __init__(self, folder='datasets', mode='training', n_classes=256, batch_size=1, resize_shape=None, 
        ...
        if mode == 'training' and self.crop_shape:
            ...
            #self.Y1 = np.zeros((batch_size, crop_shape[1]//2, crop_shape[0]//2, self.n_classes), dtype='float32') #DBG
            ...
            #self.Y1 = np.zeros((batch_size, resize_shape[1]//2, resize_shape[0]//2, self.n_classes), dtype='float32')
    ...
    def __getitem__(self, i):        
        for n, (image_path, label_path) in enumerate(zip(self.image_path_list[i*self.batch_size:(i+1)*self.batch_size], 
            ...
            #self.Y1[n] = to_categorical(cv2.resize(label, (label.shape[1]//2, label.shape[0]//2)), self.n_classes).reshape((label.shape[0]//2, label.shape[1]//2, -1))
